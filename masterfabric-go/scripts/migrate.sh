...
DB_DSN="${DB_DSN:-postgres://navgo:navgo@localhost:5432/navgo?sslmode=disable}"
...
case "${1:-}" in
    up)
        ...
        for f in "$MIGRATION_DIR"/0*.sql; do
            ...
            if [[ -n "$sql" ]]; then
                echo "$sql" | docker exec -i navgo-postgres psql -U navgo -d navgo -q 2>/dev/null \
                    && echo "  ✓ $fname" \
    ...
    down)
        ...
        if [[ -n "$sql" ]]; then
            echo "$sql" | docker exec -i navgo-postgres psql -U navgo -d navgo -q
    ...
    status)
        ...
        docker exec navgo-postgres psql -U navgo -d navgo -c "
            SELECT 
