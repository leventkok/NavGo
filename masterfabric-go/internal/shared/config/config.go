...
func Load() *Config {
	...
	return &Config{
		...
		Database: DatabaseConfig{
			...
			User:     envOrDefault("DB_USER", "navgo"),
			Password: envOrDefault("DB_PASSWORD", "navgo"),
			DBName:   envOrDefault("DB_NAME", "navgo"),
		...
		JWT: JWTConfig{
			...
			Issuer:          envOrDefault("JWT_ISSUER", "navgo"),
		},
		Kafka: KafkaConfig{
			Brokers:           envOrDefaultSlice("KAFKA_BROKERS", []string{"localhost:9092"}),
			GroupID:           envOrDefault("KAFKA_GROUP_ID", "navgo"),
